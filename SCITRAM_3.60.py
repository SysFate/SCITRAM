#####################################################################################################
### SCITRAM V2.6 - Single Cell Integrated Transcriptional RegulAtion Modeler					  ###
#####################################################################################################
...
class Param(object):
	def __init__(self):
		...
		# Path to GRN collection
		self.rootDir = Path(__file__).resolve().parent
		self.grn_dir = self.rootDir / "GRN_collection"
...
class optionGUI(tk.Frame):
	...
	def start_savedwork(self):
		...
		zip_base = os.path.splitext(os.path.basename(self.param.file_zip))[0]
		h5ad_path = os.path.join(os.getcwd(), zip_base + ".h5ad")
		pkl_path  = os.path.join(os.getcwd(), zip_base + ".pkl")
		...
		self.param.pointsize_umap = umap_cfg.get("pointsize", 6)
		...
		self.param.pointsize_ma = ma_cfg.get("pointsize", 12)
		...
		try:
			os.remove(h5ad_path)
			os.remove(pkl_path)
			print("[DEBUG] Temporary extracted files deleted (.h5ad / .pkl)")
		except Exception as e:
			print("[WARNING] Could not delete extracted files:", e)
...
class paramGUI(tk.Frame):
	...
	def __init__(self, master, controller, param):
		...
		self.Blacklist = BooleanVar(value=True)
		...
		self.cellCycleCheck = IntVar(value=1)
		...
		#ROW 2
		tk.Label(QCPanel, text='Number of reads per cell:', font="Arial 11") \
		...
		#ROW 4
		tk.Label(QCPanel, text='Filter cells (% mito counts):', font="Arial 11") \
		...
		#PANEL PLACEMENT
		QCPanel.grid(row=1, column=0, sticky=NSEW, columnspan=3, pady=10, padx=(150, 50))
		...
		logCheck = tk.Checkbutton(
			self.NormPanel, variable=self.logCheckValue,
		...
		self.NormPanel.grid(row=2, column=0, columnspan=1, pady=5, padx=(225, 50))
		...
		neighborsEntry.grid(row=4, column=3)
		tk.Label(DimPanel, text='Resolution for leiden clustering :',
				 font="Arial 11").grid(row=5, column=2)
		resolutionEntry.grid(row=5, column=3)

		DimPanel.grid(row=3, column=0, columnspan=1, pady=5, padx=(225, 50))
		...
		buttonBack = tk.Button(
			self, text="Back", width=15,
			foreground="white", background="#48729F",
			activebackground="#91BCE9",
			font="Arial 14",
			command=lambda: controller.show_frame('optionGUI')
		)
		buttonBack.grid(row=7, column=0, sticky=W, pady=30, padx=20)

		self.start_button = ttk.Button(
			self, text="Start", style="B.TButton",
			command=lambda: self.start_analysis()
		)
		self.start_button.place(x=620, y=515, width=180, height=35)
	...
	def start_analysis(self):
		...
		def ui_log(msg):
			...
			if msg.startswith("[INFO] "):
				msg_local = msg.replace("[INFO] ", "", 1)
			else:
				msg_local = msg

			log_box.insert(tk.END, msg_local + "\n", tag)
		...
		def worker():
			try:
					...
					if len(self.param.file_h5) > 1:
						...
						for i, file in enumerate(files, start=1):
							...
							try:
								...
								adata_i = self.filterCells(adata_i)
								file_shortname = os.path.basename(file)
								adata_i.obs['orig_file'] = file_shortname
								ui_log(f"[INFO] Filtered sample: {file_shortname} "
						...
						adata = self.readH5File(self.param.file_h5[0])
						adata = self.filterCells(adata)
						ui_log(f"[INFO] Filtered sample: {adata.n_obs} cells × {adata.n_vars} genes")

				elif self.param.directory_mtx:
					if len(self.param.directory_mtx) > 1:
						ui_log("[INFO] Multiple MTX directories detected — integrating.")
						adata = self.dataIntegration_mtx()
					else:
						ui_log("[INFO] Single MTX directory detected — loading directly.")
						adata = self.loadSingleMTX(self.param.directory_mtx[0])
						adata = self.filterCells(adata)
					ui_log(f"[INFO] MTX result: {adata.n_obs} cells × {adata.n_vars} genes")
				...
				if self.deleteDuplicate.get():
					try:
						adata = self.runScrublet(adata)
						ui_log("[INFO] Scrublet completed.")
					except Exception as e:
						ui_log(f"[WARNING] Scrublet failed: {e}")

				# 3) NORMALIZATION 
				ui_step("Normalization", 50)
				try:
					adata = self.normalization(adata)
				...
				try:
					...
					if 'orig_file' in adata.obs and len(set(adata.obs['orig_file'])) > 1:
						ui_log("[INFO] Running MNN batch correction...")
						try:
							result = sc.external.pp.mnn_correct(adata, batch_key='orig_file')
							if isinstance(result, sc.AnnData):
								adata = result
							elif isinstance(result, (list, tuple)) and isinstance(result[0], sc.AnnData):
								adata = result[0]
							sc.tl.pca(adata, svd_solver='arpack',
									  n_comps=self.pcNumber.get())
							adata.X = np.nan_to_num(adata.X, nan=0.0, posinf=0.0, neginf=0.0)
							ui_log("[INFO] MNN correction done.")
						except Exception as e:
							ui_log(f"[WARNING] MNN correction failed: {e}")
					else:
						ui_log("[INFO] Single sample — skipping MNN.")
	...
	# ----------------------------- Scrublet ------------------------------
	def runScrublet(self, adata):
		if not self.deleteDuplicate.get():
			return adata
		try:
			if 'orig_file' in adata.obs:
				sc.external.pp.scrublet(
					adata, batch_key="orig_file",
					n_prin_comps=20, n_neighbors=10, copy=False
				)
			else:
				sc.external.pp.scrublet(
					adata, n_prin_comps=20, n_neighbors=10, copy=False
				)
			n_before = adata.n_obs
			adata = adata[~adata.obs['predicted_doublet'], :].copy()
			n_after = adata.n_obs
			print(f"[INFO] Removed {n_before - n_after} predicted doublets "
				  f"({n_after} cells remain).")
		except Exception as e:
			msg.showwarning("WARNING", f"Scrublet failed: {e}")
		return adata

	# -------------------------- Normalization ---------------------------
	def normalization(self, adata):
		sc.pp.normalize_total(adata, target_sum=1e4, inplace=True)
		if self.logCheckValue.get():
			sc.pp.log1p(adata, copy=False)
		if self.scaleCheckValue.get():
			sc.pp.scale(adata, copy=False, zero_center=self.scaleZeroCenter.get())
		# IMPORTANT: Dont touch adata.raw
		return adata
	...
	# ---------------------- MTX integration -----------
	def dataIntegration_mtx(self):
		if len(self.param.directory_mtx) == 1 and len(os.listdir(self.param.directory_mtx[0])) > 3:
			return self.dataIntegration_mtx_one_folder()
		else:
			return self.dataIntegration_mtx_multiple_folder()

	def dataIntegration_mtx_one_folder(self):
		listData = []
		directory = self.param.directory_mtx[0]
		listPrefix = set(['_'.join(file.split('_')[0:len(file.split('_')) - 1])
						  for file in os.listdir(directory)])
		for prefix in listPrefix:
			try:
				adata = sc.read_10x_mtx(directory, make_unique=False,
										cache=True, prefix=f"{prefix}_")
			except Exception:
				continue
			adata.var_names_make_unique()
			listData.append(adata)
			print(adata)
		if len(listData) > 1:
			adata = ad.concat(listData, join='outer',
							  label='orig_file', index_unique="-",
							  keys=listPrefix)
			all_var = [x.var for x in listData]
	...
	def dataIntegration_mtx_multiple_folder(self):
		listData = []
		for directory in self.param.directory_mtx:
			try:
				adata = sc.read_10x_mtx(directory, make_unique=False, cache=True)
			except Exception:
				try:
					directoryName = os.path.basename(directory)
					adata = sc.read_10x_mtx(
						directory, make_unique=False,
						cache=True, prefix=f"{directoryName}_"
					)
				except Exception:
					continue
			adata.var_names_make_unique()
		...
		if len(listData) > 1:
			adata = ad.concat(
				...
				label='orig_file',
				index_unique="-",
				keys=[x.split("/")[-1] for x in self.param.directory_mtx]
			)
			all_var = [x.var for x in listData]
			all_var = pd.concat(all_var, join="outer")
			all_var = all_var[~all_var.duplicated()]
	...
	# ---------------------------- QC + Blacklist ------------------------
	def filterCells(self, adata):
		# Subsample (opt)
		adata = self.get_subsample(adata)

		# Basic Filters
		sc.pp.filter_genes(adata, min_cells=int(self.minCell.get()), inplace=True)
		sc.pp.filter_cells(adata, min_counts=int(self.minCount.get()), inplace=True)
		sc.pp.filter_cells(adata, max_counts=int(self.maxCount.get()), inplace=True)
		sc.pp.filter_cells(adata, min_genes=int(self.minGene.get()), inplace=True)
		sc.pp.filter_cells(adata, max_genes=int(self.maxGene.get()), inplace=True)

		# % mythocondrial
		first_gene = adata.var_names[0]
		is_mouse = first_gene.islower()
		mt_prefix = "Mt-" if is_mouse else "MT-"
		...
		adata = adata[adata.obs.pct_counts_mt < self.maxPercMito.get(), :].copy()

		# ----------------- Blacklist (before raw/layers) ----------------
		if getattr(self, 'Blacklist', None) and self.Blacklist.get():
			root_dir = Path(__file__).parent.resolve()

			# Detect species quickly
			varnames = adata.var_names
			is_human = any(name.isupper() for name in varnames[:50])
			if is_human:
				blacklist_file = os.path.join(
					root_dir, "ExtraFiles", "Human_blacklist_genes_Ensembl115.tsv"
				)
				print(f"[INFO] Using human blacklist file: {blacklist_file}")
			else:
				blacklist_file = os.path.join(
					root_dir, "ExtraFiles", "Mouse_blacklist_genes_Ensembl115.tsv"
				)
				print(f"[INFO] Using mouse blacklist file: {blacklist_file}")

			if not os.path.exists(blacklist_file):
				print(f"[WARNING] Blacklist file does not exist: {blacklist_file} — skipping blacklist.")
			else:
				try:
					blacklist_df = pd.read_csv(blacklist_file, sep='\t')
					if 'hgnc_symbol' not in blacklist_df.columns:
						raise KeyError("The column 'hgnc_symbol' doesnt exist in the blacklist.")
					blacklist_genes = (
						blacklist_df['hgnc_symbol']
						.dropna().astype(str).unique().tolist()
					)
					before_n = adata.n_vars
					adata = adata[:, ~adata.var_names.isin(blacklist_genes)].copy()
					after_n = adata.n_vars
					print(f"[INFO] Blacklist applied: {before_n - after_n} genes removed, "
						  f"{after_n} genes retained.")
				except Exception as e:
					traceback.print_exc()
					msg.showwarning("WARNING", f"Blacklist impossible to apply: {e}")
		return adata

	# ---------------------- Decorator  ---------------------------------
	def track_step(progress_win, step_name):
		def decorator(func):
			def wrapper(*args, **kwargs):
				progress_win.update_step(step_name)
...
class umapGUI(tk.Frame):
	...
	def init_frame(self):
		...
		self.listCluster = tk.Listbox(
			...
			width=8, height=18, font="Arial 14", justify='center'
		)
		...
		reclusterBtn.grid(row=0, column=3, padx=(0, 5))
	...
	def onselectCluster(self, event):
		...
		if text_selected == "all":
			...
			self.default_umap()
			return
		cluster = self.param.clusters.get(text_selected)
		if cluster is None:
...
###################################################################################
		...
		saved_n_clusters = getattr(self, "n_clusters", len(saved_clusters))
		...
		adata_save = adata_original

		# Remove colors column (saved separately)
		if "colors" in adata_save.obs.columns:
			adata_save.obs = adata_save.obs.drop(columns=["colors"])
		...
		# --------------------------------------------------
		print("[SAVE] Dropping adata.X to reduce file size.")
		adata_save.X = None
  
		# Convert all layers to sparse
		for layer in list(adata_save.layers.keys()):
			arr = adata_save.layers[layer]
			if not issparse(arr):
				try:
					adata_save.layers[layer] = csr_matrix(arr)
				except Exception as e:
					msg.showerror("Save Error", f"Layer '{layer}' sparse conversion failed:\n{e}")
					return
 
		# ----------
		#FINAL CLEANUP for lightweight .h5ad
		print("[SAVE] Performing final AnnData cleanup (SCITRAM light mode)")

		# 1) Ensure X is not saved
		#adata_save.X = None

		# Keep only UMAP in obsm
		for key in list(adata_save.obsm.keys()):
			if key != "X_umap":
				del adata_save.obsm[key]
				print(f"[SAVE] Removed obsm['{key}']")

		# Remove varm entirely (PCA loadings etc.)
		if len(adata_save.varm) > 0:
			adata_save.varm.clear()
			print("[SAVE] Cleared varm")

		# Remove obsp entirely (neighbors graph)
		if len(adata_save.obsp) > 0:
			adata_save.obsp.clear()
			print("[SAVE] Cleared obsp")

		# Remove uns (historical analysis metadata)
		if len(adata_save.uns) > 0:
			adata_save.uns.clear()
			print("[SAVE] Cleared uns")

		print("[SAVE] Final cleanup complete.")
...
##############################################################################
		...
		# Restore colors to live session
		if saved_colors is not None:
			self.param.adata.obs["colors"] = saved_colors

		# Create ZIP file
		print("[SAVE] Creating ZIP:", zip_path)
		try:
		...
		print("[SAVE] Project saved successfully →", zip_path)
	...
	def refresh_cluster_listbox(self):
		...
		for lab in labels_sorted:
			self.listCluster.insert(tk.END, lab)
...
class DiffGenesSetup(tk.Toplevel):
	...
	def build_ui(self):
		...
		cluster_names = [key for key in self.param.clusters.keys()]

		self.refCluster = StringVar(value=cluster_names)
		self.targetCluster = StringVar(value=cluster_names)
	...
	def run_analysis(self):
		ref = [self.ref_box.get(i) for i in self.ref_box.curselection()]
		targ = [self.target_box.get(i) for i in self.target_box.curselection()]
...
class DiffGenesResults(tk.Toplevel):
	...
	def __init__(self, master, param, export_manager, df, targ, ref, lfc_thr, p_thr, topN):
		super().__init__(master)
	...
	def save_table(self):
		...
		self.ExportManager.save_table(
			df_table,
			root_name=f"DEG_table_C{self.deg_targ}vsC{self.deg_ref}_LFC_{self.lfc_thr}"
		)
		...
		"""def on_configure(event=None):
			if self._plot_resize_job is not None:
				try:
					...
					pass
			self._plot_resize_job = self.after(60, resize_to_container)"""

		#self.plot_configure_bind_id = self.plot_container.bind("<Configure>", on_configure)
		#self.after(80, resize_to_container)
	...
	def draw_ma_plot(self, fig, ax, df, targ, ref, topN):
		...
		ax.set_title(f"Cluster {targ} vs Cluster {ref} (MA plot)")
	...
	def draw_volcano_plot(self, fig, ax, df, targ, ref, topN, lfc_thr, p_thr):
		...
		ax.set_title(f"Cluster {targ} vs Cluster {ref} (Volcano plot)")
		...
		try:
			...
			if hasattr(self, "fig") and hasattr(self, "canvas"):
				try:
					...
					print(f"[WARNING] UMAP clear failed: {e}")

				# 7) Auto point size based on number of cells
				n = self.param.adata.n_obs
				point_size = self.auto_point_size(n)

				# 8) Draw the updated UMAP
				...
				self.fig.colorbar(sca, shrink=0.6, fraction=0.03)
...
class networkOption(tk.Toplevel):
	def __init__(self, master, controller, param):
		...
		self.refCluster = StringVar(value=[key for key in self.param.clusters.keys()])
		self.targetCluster = StringVar(value=[key for key in self.param.clusters.keys()])
		self.grid_columnconfigure(0, weight=1)
		...
		self.refClusterList = tk.Listbox(selectCellCluster, listvariable=self.refCluster,selectmode=tk.MULTIPLE, width=7, height=10, font="Arial 11", justify='center', exportselection=0)
		self.targetClusterList = tk.Listbox(selectCellCluster, listvariable=self.targetCluster,selectmode=tk.MULTIPLE, width=7, height=10, font="Arial 11", justify='center', exportselection=0)
	...
	def parse_clusters(self):
		...
		refClusters = [self.refClusterList.get(i) for i in self.refClusterList.curselection()]
		targetClusters = [self.targetClusterList.get(i) for i in self.targetClusterList.curselection()]
		...
		self.info += f"Reference clusters: {refClusters} | Target clusters: {targetClusters}\n"
...
# Dynamic style mappings for LFC
	...
	def plot_network_html(self):
		...
		# Output directory
		outdir = Path(__file__).parent / "Network"
		outdir.mkdir(exist_ok=True)
		timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

		target_clusters = "_".join(self.targetClusterList.get(i) for i in self.targetClusterList.curselection())
		ref_clusters = "_".join(self.refClusterList.get(i) for i in self.refClusterList.curselection())
		lfc_value = self.posLFC.get()

		# Paths
		json_path = outdir / f"network_C{target_clusters}_vs_C{ref_clusters}_LFC{lfc_value}_{timestamp}.json"
		html_path = outdir / f"network_C{target_clusters}_vs_C{ref_clusters}_LFC{lfc_value}_{timestamp}.html"
...
class dummyOptions():
	def __init__(self, networkOptionsObject):
		...
		self.p_value = networkOptionsObject.p_value
		self.keep_starting_neutral = networkOptionsObject.keep_starting_neutral
		self.n_iteration = networkOptionsObject.n_iteration
		self.refClusterList=networkOptionsObject.refClusterList
		self.targetClusterList=networkOptionsObject.targetClusterList
		self.networkParamList=networkOptionsObject.networkParamList
		self.data = networkOptionsObject.data
		self.co_network=networkOptionsObject.co_network
		self.lfc = networkOptionsObject.lfc
		self.info = networkOptionsObject.info
